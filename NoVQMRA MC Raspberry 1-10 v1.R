...
###################################
## Note: for raspberry analysis there is no influence of the packer on produce contamination
...
nvhG<-(nvhH - nvhH*((1-Trhhg)*Phwg))*(2.1/245)
...
nvpG<-(nvpH - nvpH*((1-Trphg)*Ppwg))*(2.1/245)
...
#NoV on produce after contacting clamshell
nvhP2<-nvhP1-nvBi
...
nvpP22decayl <- nvhP23/(10^(norodecaylow)*7)
nvpP23decayl <- nvhP25/(10^(norodecaylow)*7)
nvpP24decayl <- nvhP27/(10^(norodecaylow)*7)
nvpP25decayl <- nvhP29/(10^(norodecaylow)*7)
nvpP26decayl <- nvhP31/(10^(norodecaylow)*7)
nvpP27decayl <- nvhP33/(10^(norodecaylow)*7)
nvpP28decayl <- nvhP35/(10^(norodecaylow)*7)
nvpP29decayl <- nvhP37/(10^(norodecaylow)*7)
nvpP210decayl <-nvhP39/(10^(norodecaylow)*7)
...
dose2rasp <- (bodyweight*berryconsumpt*nvpP22decayl)/weightberry
dose3rasp <- (bodyweight*berryconsumpt*nvpP23decayl)/weightberry
dose4rasp <- (bodyweight*berryconsumpt*nvpP24decayl)/weightberry
dose5rasp <- (bodyweight*berryconsumpt*nvpP25decayl)/weightberry
dose6rasp <- (bodyweight*berryconsumpt*nvpP26decayl)/weightberry
dose7rasp <- (bodyweight*berryconsumpt*nvpP27decayl)/weightberry
dose8rasp <- (bodyweight*berryconsumpt*nvpP28decayl)/weightberry
dose9rasp <- (bodyweight*berryconsumpt*nvpP29decayl)/weightberry
dose10rasp <- (bodyweight*berryconsumpt*nvpP210decayl)/weightberry
...
riskfracpoissR2 <- P*(1-exp(-dose2rasp/mu))
riskfracpoissR3 <- P*(1-exp(-dose3rasp/mu))
riskfracpoissR4 <- P*(1-exp(-dose4rasp/mu))
riskfracpoissR5 <- P*(1-exp(-dose5rasp/mu))
riskfracpoissR6 <- P*(1-exp(-dose6rasp/mu))
riskfracpoissR7 <- P*(1-exp(-dose7rasp/mu))
riskfracpoissR8 <- P*(1-exp(-dose8rasp/mu))
riskfracpoissR9 <- P*(1-exp(-dose9rasp/mu))
riskfracpoissR10 <- P*(1-exp(-dose10rasp/mu))
...
QMRAberry<-mc(conc,fh,hweff, Phhw,Phwg, prevh, concp, fhp, hweffp, Pphw, Ppwg, prevp, Trhhp, Trhgp,Trpph,Trphp,Trpgp, P, mu,
              ...
              dose1rasp, dose2rasp, dose3rasp, dose4rasp, dose5rasp, dose6rasp, dose7rasp, dose8rasp, dose9rasp, dose10rasp,
              riskfracpoissR2, riskfracpoissR3, riskfracpoissR4, riskfracpoissR5, riskfracpoissR6, riskfracpoissR7, riskfracpoissR8, riskfracpoissR9, riskfracpoissR10,riskfracpoissR1)
summary(QMRAberry)
summary(QMRAberry$riskfracpoissR1)
summary(QMRAberry$nvpP2)

torberry<-tornado(QMRAberry)
plot(torberry)
print(torberry)
mcratio(QMRAberry)
uncberry=tornadounc(QMRAberry)
uncberry
####################################################################################################################################################################################################################################################################
